...
{
    SGL_LOG_INFO("keyboard clicked ascii = %d", sgl_keyboard_get_opcode(event->obj));
}
...
void slider_callback(sgl_event_t *event)
{
    ...
    sprintf(buf, "H:%d%%", sgl_slider_get_value(event->obj));
...
int main(int argc, char *argv[])
{
    ...
    sgl_rect_set_alpha(rect, 255);
    sgl_obj_set_clickable(rect);

    sgl_obj_t *rect2 = sgl_rect_create(rect);
    sgl_obj_set_pos(rect2, 20, 20);
    sgl_obj_set_size(rect2, 400, 400);
    sgl_rect_set_color(rect2, SGL_COLOR_BLUE);
    sgl_obj_set_clickable(rect2);
    sgl_rect_set_radius(rect2, 0);
    sgl_rect_set_border_width(rect2, 5);
    sgl_rect_set_border_color(rect2, SGL_COLOR_RED);
    ...
    sgl_rect_set_alpha(rect3, 128);
    sgl_rect_set_color(rect3, SGL_COLOR_GRAY);
    ...
    sgl_circle_set_border_width(circle, 0);
    sgl_circle_set_radius(circle, 49);
    sgl_circle_set_pixmap(circle, &test_pixmap);

    sgl_obj_t *numbberkbd = sgl_numberkbd_create(NULL);
    sgl_obj_set_pos(numbberkbd, 400, 110);
    sgl_obj_set_size(numbberkbd, 300, 200);
    sgl_numberkbd_set_text_font(numbberkbd, &song23);
    sgl_numberkbd_set_btn_radius(numbberkbd, 10);
    ...
    sgl_ring_set_radius(ring, 44, 49);
    sgl_ring_set_color(ring, SGL_COLOR_LIGHT_GRAY);
    ...
    sgl_textbox_set_text_font(textbox, &consolas23);
    sgl_textbox_set_text(textbox, text_string);
